...
cocos2d::Scene* Nivel1::createScene()
{
...
{
	//Musica
	musica = AudioEngine::play2d("MusicaNiveles.mp3", true, .8);
	score = 10000;
	...
	listener->onKeyPressed = CC_CALLBACK_2(Nivel1::onKeyPressed, this);
	listener->onKeyReleased = CC_CALLBACK_2(Nivel1::onKeyReleased, this);
	...
	//Agregar titulo
	auto label = Label::createWithTTF("Level 1", "fonts/Marker Felt.ttf", 50);
	...
	this->scheduleUpdate();
	this->schedule(CC_SCHEDULE_SELECTOR(Nivel1::funcionRandom), .1);
	this->schedule(CC_SCHEDULE_SELECTOR(Nivel1::funcionCarritos), .4);
	this->schedule(CC_SCHEDULE_SELECTOR(Nivel1::funcionEstrella), 5);
	this->schedule(CC_SCHEDULE_SELECTOR(Nivel1::funcionRecogerEstrella), .1);
	this->schedule(CC_SCHEDULE_SELECTOR(Nivel1::funcionChoque), .1);
	this->schedule(CC_SCHEDULE_SELECTOR(Nivel1::funcionFondo), .8);
	this->schedule(CC_SCHEDULE_SELECTOR(Nivel1::Score), .5);
	this->schedule(CC_SCHEDULE_SELECTOR(Nivel1::Seconds), 1);
...
void Nivel1::funcionRandom(float detla)
{
	posCarril = 1 + rand() % 4;
	num = rand() % 6;
...
void Nivel1::funcionCarritos(float delta)
{
	...
	auto avanzarCarrito = MoveBy::create(3, Vec2(0, -1200));
...
void Nivel1::funcionFondo(float delta)
{
...
void Nivel1::funcionEstrella(float delta)
{
...
void Nivel1::update(float deltaTime)
{
	...
	{
		auto nivel2 = Scene::createWithPhysics();
		nivel2->addChild(Nivel2::createScene());
...
void Nivel1::onKeyPressed(cocos2d::EventKeyboard::KeyCode key, cocos2d::Event* event)
{
	auto visibleSize = Director::getInstance()->getVisibleSize();
...
void Nivel1::onKeyReleased(cocos2d::EventKeyboard::KeyCode key, cocos2d::Event* event)
{
	if (key == EventKeyboard::KeyCode::KEY_RIGHT_ARROW)
	{
		R = false;
...
bool Nivel1::onContactBegin(PhysicsContact& contact)
{
...
void Nivel1::funcionRecogerEstrella(float delta)
{	
	if (recogerEstrella)
	{
		contadorEstrella++;
		estrella->setVisible(false);
		estrella->setPosition(Vec2(500, 500));
...
void Nivel1::funcionChoque(float delta)
{
	if (choqueC)
	{
		if (score - 500 < 0)
...
void Nivel1::Seconds(float delta)
{
	aux++;
}
